...
public class Renter {
    ...
    private static void receiveResultsFromMaster() {
        try (ServerSocket serverSocket = new ServerSocket(7777)) {
            System.out.println("Listening for results on port 7777");
            ...
            if (!resultString.isEmpty()) {
                ...
                System.out.println(formatResults(new JSONObject(resultString)));
            } else {
                System.out.println("No results received from master.");
            }
            clientSocket.close();
        } catch (IOException e) {
            System.out.println("Error receiving results from master: " + e.getMessage());
        }
    }

    private static String formatResults(JSONObject json) {
        StringBuilder formattedResults = new StringBuilder();
        int mapId = json.getInt("mapId");
        JSONArray data = json.getJSONArray("data");
        formattedResults.append("Results for mapId ").append(mapId).append(":\n");

        for (int i = 0; i < data.length(); i++) {
            JSONObject room = new JSONObject(data.getString(i));
            formattedResults.append("Room Name: ").append(room.getString("roomName")).append("\n")
                    .append("Area: ").append(room.getString("area")).append("\n")
                    .append("No. of Persons: ").append(room.getInt("noOfPersons")).append("\n")
                    .append("Stars: ").append(room.getInt("stars")).append("\n")
                    .append("No. of Reviews: ").append(room.getInt("noOfReviews")).append("\n")
                    .append("Room Image: ").append(room.getString("roomImage")).append("\n")
                    .append("Price Per Night: ").append(room.getInt("pricePerNight")).append("\n")
                    .append("Available Dates: ").append(room.getJSONArray("availableDates").toString()).append("\n\n");
        }
