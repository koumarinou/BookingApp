    ...
    try {
        ...
        if (jsonObject.has("type")) {
            ...
            if (requestType.equals("search")) {
                ...
                // If it's a reservation type, forward to the appropriate worker node
                String roomName = jsonObject.getString("roomName");
                WorkerNode node = selectWorkerNode(roomName);
                if (node != null) {
                    forwardDataToWorker(node, jsonData.toString());
                } else {
                    System.out.println("No valid worker node found to forward reservation data.");
                }
            } else {
                // Handle other types of room data, extract room name and forward to the appropriate worker node
