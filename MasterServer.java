...
public class MasterServer {
    ...
    private static ConcurrentHashMap<Integer, String> resultMap = new ConcurrentHashMap<>();
    ...
    private static void handleResult(Socket socket) {
        try (BufferedReader in = new BufferedReader(new InputStreamReader(socket.getInputStream()))) {
            ...
            if (data.length() > 0) {
                resultMap.put(mapId, json.toString());
                sendResultsToRenter(json.toString());
            } else {
                System.out.println("No results to send for mapId " + mapId);
            }
        } catch (IOException e) {
    ...
    private static void sendResultsToRenter(String resultData) {
        try (Socket renterSocket = new Socket("127.0.0.1", 7777);
             PrintWriter out = new PrintWriter(renterSocket.getOutputStream(), true)) {
            out.println(resultData);
            System.out.println("Results sent to renter.");
        } catch (IOException e) {
            System.out.println("Error sending results to renter: " + e.getMessage());
        }
    }
}
